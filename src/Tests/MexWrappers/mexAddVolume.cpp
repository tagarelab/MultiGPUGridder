...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 4)
    {
        mexErrMsgTxt("mexAddVolume: There should be 4 inputs, (1) VolumeOne as a single array, (2) VolumeTwo as a single array, (3) Volume size as int32, and (4) GPU device number as int32.");
    }
    ...
    AddFilter->SetVolumeSize(VolumeSize[0]);
    AddFilter->SetNumberOfSlices(VolumeSize[2]);
    AddFilter->SetVolumeOne(d_VolumeOne->GetPointer());
    AddFilter->SetVolumeTwo(d_VolumeTwo->GetPointer());
    AddFilter->Update();
