...
| Python       | >= 3.*    | Optional: If compiling the Python wrappers for calling the class.|
...
## CUDA ASYNCHRONOUS MEMORY TRANSFERS
...
## Matlab Wrapper - Memory Persistance

First, we needed to have the host memory persistant when going back to Matlab in order only need to allocate both the host and device memory once.
...
## Matlab Wrapper - In-place computation
