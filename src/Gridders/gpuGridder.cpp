...
void gpuGridder::InitilizeGPUArrays()
{
    ...
    this->d_CoordAxes = new DeviceMemory<float>(this->h_CoordAxes->GetDim(), this->h_CoordAxes->GetSize(), this->GPU_Device);
    this->d_CoordAxes->AllocateGPUArray();

    // Allocate the Kaiser bessel lookup table
    this->d_KB_Table = new DeviceMemory<float>(this->h_KB_Table->GetDim(), this->h_KB_Table->GetSize(), this->GPU_Device);
