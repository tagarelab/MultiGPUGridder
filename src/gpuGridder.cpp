...
void gpuGridder::CopyVolumeToGPU()
{
    ...
    cudaMemcpy(this->d_CASVolume->ptr, this->CASVolume->ptr, this->CASVolume->bytes(), cudaMemcpyHostToDevice);
}
...
void gpuGridder::KB_Table_To_GPU()
{
    ...
    Log("this->ker_bessel_Vector->length()");
    Log(this->ker_bessel_Vector->length());
    ...
    // for (int i = 0; i < this->ker_bessel_Vector->length(); i++)
    // {
    //     this->ker_bessel_Vector[i] = ker_bessel_Vector[i];
    //     Log(this->ker_bessel_Vector->ptr[i]);
    // }

    cudaMemcpy(this->d_KB_Table->ptr, this->ker_bessel_Vector->ptr, this->ker_bessel_Vector->bytes(), cudaMemcpyHostToDevice);
}
